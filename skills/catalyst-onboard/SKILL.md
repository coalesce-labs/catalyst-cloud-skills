...
- **Provider consent belongs to the person in a browser.** Use `catalyst-skills connections personal <linear|github> start`, give the person its URL, then check `status`. Start personal Linear consent after tenant Linear is connected. Start personal GitHub consent only after the tenant GitHub App is installed and its repository is registered. Never claim a grant succeeded because a browser opened.
- **Use the team CLI for team setup.** `team check`, `map`, `adopt`, `migrate`, and `checklist` use the member's own login. Show each preview to the person. Pass `--yes` only after they approve that plan. A changed plan needs another review. Repository registration and repository environment remain settings-page work.
