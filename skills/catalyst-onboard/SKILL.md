...
- **Some steps a key cannot do yet.** Listing every project, saving a stage mapping, adopting the workflow and registering a repository are settings-page work today; a key-callable path for them is being built. Route those through the browser and say plainly that it is a gap, not the design. Never guess at a route for them.
