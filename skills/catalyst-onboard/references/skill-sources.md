...
3. If the old plugin is active, remove only `catalyst-dev@catalyst` through Claude Code's plugin manager. If old skill copies are present, remove only copies whose recorded source is the deprecated `coalesce-labs/catalyst` repository. Keep unrelated skills and plugin installs.
